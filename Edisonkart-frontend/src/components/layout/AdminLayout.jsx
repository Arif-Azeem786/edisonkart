...
const AdminLayout = () => {
  ...
  // Redirect restricted roles away from restricted pages
  useEffect(() => {
    if (isEmployee || isVendor) {
      const restrictedPaths = ['/admin', '/admin/users']
      if (isVendor) restrictedPaths.push('/admin/categories', '/admin/contact')
      
      if (restrictedPaths.includes(location.pathname)) {
        navigate('/admin/orders', { replace: true })
      }
    }
  }, [location.pathname, isEmployee, isVendor, navigate])
  ...
  const menuItems = allMenuItems.filter(item => {
    if (isVendor) {
      return ['/admin/orders', '/admin/products'].includes(item.path)
    }
    if (isEmployee) {
  ...
  // Helper to get consistent title
  const getPageTitle = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      >
        {/* Top Gradient Line matching Navbar */}
          ...
          <Link to="/" className="flex items-center gap-3 group w-full">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-medium text-[#F97316] uppercase tracking-widest -mt-0.5">
                {isEmployee ? 'Employee Panel' : isVendor ? 'Vendor Panel' : 'Admin Panel'}
              </span>
            </div>
          </Link>
        </div>

        {/* Navigation — always show labels on mobile */}
        ...
        {/* User Profile & Logout — always show on mobile */}
